...
export function CreateWodPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setIsSaving(true);
    try {
      setSavedWod(await createUserWod(body, token));
    } catch (caughtError: unknown) {
    ...
    <section className="create-wod-page">
      ...
      <header className="page-heading create-wod-heading">
        <div className="page-heading__body">
          <h1>Crear WOD</h1>
          <p className="heading-support">Construye una sesión que tenga sentido para tu entrenamiento.</p>
        </div>
      ...
      <div className="create-wod-layout">
        <form className="form-panel create-wod-form" onSubmit={handleSubmit} noValidate aria-busy={isSaving}>
          <div className="form-heading">
            ...
            <strong>La ficha empieza aquí</strong>
            <p>Completa la estructura y añade los movimientos que quieres repetir.</p>
          </div>
          ...
          {savedWod && <StateMessage kind="success" title="WOD guardado" message={`“${savedWod.name}” ya forma parte de tus WODs. Identificador ${savedWod.id}.`} action={{ label: "Crear otro WOD", onClick: resetForm }} />}
          <button className="button button--accent button--wide create-wod-submit" type="submit" disabled={isSaving}>{isSaving ? "Guardando…" : "Guardar WOD"}</button>
        </form>
