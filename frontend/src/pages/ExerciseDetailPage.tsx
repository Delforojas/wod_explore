     ...
     <section className="detail-page detail-page--exercise">
      ...
      <div className="detail-grid">
        <article className="detail-manifest">
          ...
          <section className="result-list" aria-labelledby="exercise-results-title"><div className="section-heading"><h2 id="exercise-results-title">Tus marcas</h2><span>{results.length}</span></div>{results.length === 0 ? <StateMessage kind="empty" title="Todavía no tienes marcas para este ejercicio." /> : <ul className="result-items">{results.map((result) => <li className="result-row" key={result.id}><strong>{result.value} {UNIT_LABELS[result.unit]}</strong><span>{RECORD_TYPE_LABELS[result.recordType]}</span><small>{dateFormatter.format(new Date(result.performedAt))}</small></li>)}</ul>}</section>
        </article>
